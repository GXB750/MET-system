...
const devs = ['351369612832669697'];
const db = require('quick.db');
const premium = ['351369612832669697']
...
client.on('ready', function(){
    ...
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[i],`طاط تعال خاص`);
    }, ms);
...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'Community MET ©\'s translator',
...
if (message.content.startsWith(prefix + 'tag')) {
    let args = message.content.split(" ").slice(1);
if(!args[0]) return message.reply('نرجو كتابة النص الذي تريده');  
...
message.channel.send(
  ...
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/MaS7pun**")
...
client.users.get("351369612832669697").send(
    "\n" + "**" + "? السيرفر :" + "**" +
    "\n" + "**" + "» " + message.guild.name + "**" +
    "\n" + "**" + " ? المرسل : " + "**" +
    "\n" + "**" + "» " + message.author.tag + "**" +
